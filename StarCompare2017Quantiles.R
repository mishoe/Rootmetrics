...
#SMS parameters
#text send success upper and lower
tss.up<-1#.99 #10th percentile
tss.low<-.996#.97 #50th percentile
#lite data secure
lds2<-.991#.98 #50th percentile
#Lite data secure Task Success
ldsts<-.968#.97 #50th percentile




#Call reliability Parameters
#mobile to landline call drop rate
m2ld.5<-.003#.03 # earn .5 stars #50th percentile
m2ld1.0<-.0022#.025 #earn 1 star #60th percentile
m2ld1.5<-.0015#.02 #earn 1.5 stars #70th percentile
m2ld2.0<-.0011#.015 #earn 2 stars #80th percentile
m2ld2.5<-0#.01 #earn 2.5 stars #90th percentile

#mobile to mobile call block rate
m2lb.5<-.0030#.01 #earn .5 stars
m2lb1.0<-.0011#.005 #earn 1 star
m2lb1.5<-0#.002 #earn 1.5 stars

#mobile to Mobile call block rate
m2mb.5<-.011#.02 #earn .5 stars
m2mb1.0<-.0027#.015 #earn 1 star
...
ldsts<-ldsts #lite data secure task success
dts<-.986#.99 #download task success #50th percentile
#download throughput
dlt.5<-.9510#.9 #earn .5 stars #50th percentile
dlt1.0<-.9682#.92 #earn 1 star #70th percentile
dlt1.5<-.9756#.95 #earn 1.5 stars #80th percentile
dlt2.0<-.9828#.97 #earn 2 stars #90th percentile
uts<-.9707#.99 #upload task success #50th percentile
ult.5<-.9278#.92 #uplaod throughput .5 stars #50th percentile
ult1.0<-.9713#.97 #upload throughput 1 star #90th percentile
udpdr<-.1357#.05 #udp packet drop rate #50th percentile


#speed and performance parameters
#download throughput 5th percentile
dlt5th.5<-1260#2 #mb/sec .5 stars #50th percentile
dlt5th1.0<-2189#3 #mb/sec 1 star #75th Percentile
dlt5th1.5<-3061#5 #mb/sec 1.5 stars #90th percentile
#time to first byte
ttfbd.5<-.377#.7 #sec .5 stars #50th percentile
ttfb1.0<-.304#.4 #sec 1 star#90th percentile
#download throughput 95th percentile
dlt95th<-70584#75 #mb/sec #50th percentile
#upload throughput 5th percentile
ult5th.5<-389#1 #mb/sec .5 stars #50th percentile
ult5th1.0<-1341#1.5 #mb/sec 1 star #90th percentile
# lite data 95th percentile
ldsmt95th<-.402#1 #sec #50th percentile
# mobile to mobile call success time 95th percentile
m2mcs95th<-3.561#7 #sec #50th percentile
...
for(carrier_id in 1:length(carriers)){
  ...
  for (i in 1:nrow(locat_corr)){
    ...
    smsStars_1[i] = smsStars_1[i]+if(market_data_1$`sms_access_success_inter`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_access_success_inter`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsStars_1[i] = smsStars_1[i]+if(market_data_1$`sms_access_success_intra`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_access_success_intra`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsStars_1[i] = smsStars_1[i]+if(market_data_1$`sms_task_success_inter`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_task_success_inter`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsStars_1[i] = smsStars_1[i]+if(market_data_1$`sms_task_success_intra`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_task_success_intra`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsStars_1[i] = smsStars_1[i]+if(market_data_1$`ldrs_task_success`[data_ind_1[i]]>=ldsts){.5}else{0}
    
    smsStars_2[i] = smsStars_2[i]+if(market_data_2$`sms_access_success_inter`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_access_success_inter`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsStars_2[i] = smsStars_2[i]+if(market_data_2$`sms_access_success_intra`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_access_success_intra`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsStars_2[i] = smsStars_2[i]+if(market_data_2$`sms_task_success_inter`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_task_success_inter`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsStars_2[i] = smsStars_2[i]+if(market_data_2$`sms_task_success_intra`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_task_success_intra`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsStars_2[i] = smsStars_2[i]+if(market_data_2$`ldrs_task_success`[data_ind_2[i]]>=ldsts){.5}else{0}
    
    smsVector[1]=smsVector[1]+if(market_data_1$`sms_access_success_inter`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_access_success_inter`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsVector[2]=smsVector[2]+if(market_data_1$`sms_access_success_intra`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_access_success_intra`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsVector[3]=smsVector[3]+if(market_data_1$`sms_task_success_inter`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_task_success_inter`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsVector[4]=smsVector[4]+if(market_data_1$`sms_task_success_intra`[data_ind_1[i]]>=tss.up){1}else if(market_data_1$`sms_task_success_intra`[data_ind_1[i]]>=tss.low){.5}else{0}
    smsVector[5]=smsVector[5]+if(market_data_1$`ldrs_task_success`[data_ind_1[i]]>=ldsts){.5}else{0}
    smsVector[6]=smsVector[6]+if(market_data_2$`sms_access_success_inter`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_access_success_inter`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsVector[7]=smsVector[7]+if(market_data_2$`sms_access_success_intra`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_access_success_intra`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsVector[8]=smsVector[8]+if(market_data_2$`sms_task_success_inter`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_task_success_inter`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsVector[9]=smsVector[9]+if(market_data_2$`sms_task_success_intra`[data_ind_2[i]]>=tss.up){1}else if(market_data_2$`sms_task_success_intra`[data_ind_2[i]]>=tss.low){.5}else{0}
    smsVector[10]=smsVector[10]+if(market_data_2$`ldrs_task_success`[data_ind_2[i]]>=ldsts){.5}else{0}
    ...
    smsStars_1[i]<-smsStars_1[i]+if(indTest<=lds2){.5}else{0}
    smsVector[11]=smsVector[11]+if(indTest<=lds2){.5}else{0}
    ...
    smsStars_2[i]<-smsStars_2[i]+if(indTest<=lds2){.5}else{0}
    smsVector[12]=smsVector[12]+if(indTest<=lds2){.5}else{0}
    
    #mobile to landline call drop
    callStars_1[i]=callStars_1[i]+ifelse(market_data_1$co_drop[data_ind_1[i]]<=m2ld2.5,2.5,ifelse(market_data_1$co_drop[data_ind_1[i]]<=m2ld2.0,2,ifelse(market_data_1$co_drop[data_ind_1[i]]<=m2ld1.5,1.5,ifelse(market_data_1$co_drop[data_ind_1[i]]<=m2ld1.0,1,ifelse(market_data_1$co_drop[data_ind_1[i]]<=m2ld.5,.5,0)))))
    #mobile to landline call block
    callStars_1[i]=callStars_1[i]+ifelse(market_data_1$co_block[data_ind_1[i]]<=m2lb1.5,1.5,ifelse(market_data_1$co_block[data_ind_1[i]]<=m2lb1.0,1,ifelse(market_data_1$co_block[data_ind_1[i]]<=m2lb.5,.5,0)))
    #mobile to landline call block
    callStars_1[i]=callStars_1[i]+ifelse(market_data_1$m2mo_block[data_ind_1[i]]<=m2mb1.0,1,ifelse(market_data_1$m2mo_block[data_ind_1[i]]<=m2mb.5,.5,0))

    #mobile to landline call drop
    callStars_2[i]=callStars_2[i]+ifelse(market_data_2$co_drop[data_ind_2[i]]<=m2ld2.5,2.5,ifelse(market_data_2$co_drop[data_ind_2[i]]<=m2ld2.0,2,ifelse(market_data_2$co_drop[data_ind_2[i]]<=m2ld1.5,1.5,ifelse(market_data_2$co_drop[data_ind_2[i]]<=m2ld1.0,1,ifelse(market_data_2$co_drop[data_ind_2[i]]<=m2ld.5,.5,0)))))
    #mobile to landline call block
    callStars_2[i]=callStars_2[i]+ifelse(market_data_2$co_block[data_ind_2[i]]<=m2lb1.5,1.5,ifelse(market_data_2$co_block[data_ind_2[i]]<=m2lb1.0,1,ifelse(market_data_2$co_block[data_ind_2[i]]<=m2lb.5,.5,0)))
    #mobile to landline call block
    callStars_2[i]=callStars_2[i]+ifelse(market_data_2$m2mo_block[data_ind_2[i]]<=m2mb1.0,1,ifelse(market_data_2$m2mo_block[data_ind_2[i]]<=m2mb.5,.5,0))
 

    #Lite Data Secure
    dataStars_1[i]=dataStars_1[i]+ifelse(market_data_1$ldrs_task_success[data_ind_1[i]]>=ldsts,.5,0)
    # Download Task Success
    dataStars_1[i]=dataStars_1[i]+ifelse(market_data_1$dsd_task_success[data_ind_1[i]]>=dts,.5,0)
    ...
    # % DL Throughput
    dataStars_1[i]=dataStars_1[i]+ifelse(percentDLThrough>=dlt2.0,2,ifelse(percentDLThrough>=dlt1.5,1.5,ifelse(percentDLThrough>=dlt1.0,1,ifelse(percentDLThrough>=dlt.5,.5,0))))
    # Upload Task
    dataStars_1[i]=dataStars_1[i]+ifelse(market_data_1$dsd_task_success[data_ind_1[i]]>=uts,.5,0)
    ...
    dataStars_1[i]=dataStars_1[i]+ifelse(percentULThrough>=ult1.0,1,ifelse(percentULThrough>=ult.5,.5,0))
    ...
    dataStars_1[i]=dataStars_1[i]+ifelse(UDP_dat_mean<=udpdr,.5,0)
    
    
    #Lite Data Secure
    dataStars_2[i]=dataStars_2[i]+ifelse(market_data_2$ldrs_task_success[data_ind_2[i]]>=ldsts,.5,0)
    # Download Task Success
    dataStars_2[i]=dataStars_2[i]+ifelse(market_data_2$dsd_task_success[data_ind_2[i]]>=dts,.5,0)
    ...
    # % DL Throughput
    dataStars_2[i]=dataStars_2[i]+ifelse(percentDLThrough>=dlt2.0,2,ifelse(percentDLThrough>=dlt1.5,1.5,ifelse(percentDLThrough>=dlt1.0,1,ifelse(percentDLThrough>=dlt.5,.5,0))))
    # Upload Task
    dataStars_2[i]=dataStars_2[i]+ifelse(market_data_2$dsd_task_success[data_ind_2[i]]>=uts,.5,0)
    # % UL Through (dsu_effective_upload_test_speed >= 500)/count(dsu_effective_upload_test_speed) -- where dsdu_effective_upload_test_speed not NULL
    ...
    dataStars_2[i]=dataStars_2[i]+ifelse(percentULThrough>=ult1.0,1,ifelse(percentULThrough>=ult.5,.5,0))
    
    #UDP Packet Drop Rate mean(udp_packet_drop_rate) -- where test_type_id = 25
    UDP_dat_mean=mean(na.omit(tmp_testDat_2[which(tmp_testDat_2$test_type_id==25),]$udp_avg_packet_drop))
    dataStars_2[i]=dataStars_2[i]+ifelse(UDP_dat_mean<=udpdr,.5,0)
    
    
    #Calculate Speed and Performance stars for each of the regions
    speedStars_1[i]=speedStars_1[i]+ifelse(market_data_1$dsd_effective_throughput_05p[data_ind_1[i]]>=dlt5th1.5,1.5,ifelse(market_data_1$dsd_effective_throughput_05p[data_ind_1[i]]>=dlt5th1.0,1,ifelse(market_data_1$dsd_effective_throughput_05p[data_ind_1[i]]>=dlt5th.5,0.5,0)))
    
    speedStars_1[i]=speedStars_1[i]+ifelse(market_data_1$dsd_time_to_first_byte_50p[data_ind_1[i]]<=ttfb1.0,1,ifelse(market_data_1$dsd_time_to_first_byte_50p[data_ind_1[i]]<=ttfbd.5,0.5,0))
    
    speedStars_1[i]=speedStars_1[i]+ifelse(market_data_1$dsd_effective_throughput_95p[data_ind_1[i]]>=dlt95th,.5,0)
    
    speedStars_1[i]=speedStars_1[i]+ifelse(market_data_1$dsu_effective_throughput_05p[data_ind_1[i]]>=ult5th1.0,1,ifelse(market_data_1$dsu_effective_throughput_05p[data_ind_1[i]]>=ult5th.5,0.5,0)) 
    
    liteData95Quant=quantile(na.omit(tmp_testDat_1[which(tmp_testDat_1$test_type_id==26),]$ldrs_task_speed_max),probs=c(.95))
    speedStars_1[i]=speedStars_1[i]+ifelse(liteData95Quant<=ldsmt95th,.5,0)
    ...
    speedStars_1[i]=speedStars_1[i]+ifelse(MM95Quant<=m2mcs95th,.5,0)
    
    #Calculate Speed and Performance stars for each of the regions
    speedStars_2[i]=speedStars_2[i]+ifelse(market_data_2$dsd_effective_throughput_05p[data_ind_2[i]]>=dlt5th1.5,1.5,ifelse(market_data_2$dsd_effective_throughput_05p[data_ind_2[i]]>=dlt5th1.0,1,ifelse(market_data_2$dsd_effective_throughput_05p[data_ind_2[i]]>=dlt5th.5,0.5,0)))
    
    speedStars_2[i]=speedStars_2[i]+ifelse(market_data_2$dsd_time_to_first_byte_50p[data_ind_2[i]]<=ttfb1.0,1,ifelse(market_data_2$dsd_time_to_first_byte_50p[data_ind_2[i]]<=ttfbd.5,0.5,0))
    
    speedStars_2[i]=speedStars_2[i]+ifelse(market_data_2$dsd_effective_throughput_95p[data_ind_2[i]]>=dlt95th,.5,0)
    
    speedStars_2[i]=speedStars_2[i]+ifelse(market_data_2$dsu_effective_throughput_05p[data_ind_2[i]]>=ult5th1.0,1,ifelse(market_data_2$dsu_effective_throughput_05p[data_ind_2[i]]>=ult5th.5,0.5,0)) 
    
    liteData95Quant=quantile(na.omit(tmp_testDat_2[which(tmp_testDat_2$test_type_id==26),]$ldrs_task_speed_max),probs=c(.95))
    speedStars_2[i]=speedStars_2[i]+ifelse(liteData95Quant<=ldsmt95th,.5,0)
    ...
    speedStars_2[i]=speedStars_2[i]+ifelse(MM95Quant<=m2mcs95th,.5,0)
  }
